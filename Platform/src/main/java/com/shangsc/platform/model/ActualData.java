...
public class ActualData extends BaseActualData<ActualData> {
    ...
    public Page<ActualData> getActualDataPage(int page, int rows, String keyword, String orderbyStr) {
        ...
        StringBuffer sqlExceptSelect = new StringBuffer(" from (" +
                "select tad.*,tc.name as companyName,tc.real_code,tc.water_unit,tc.county,twm.line_num,twm.waters_type,twm.memo,twm.vender from (select * from t_actual_data order by write_time desc)  tad " +
                ...
                "tad.write_time,tad.voltage,companyName,tc.real_code,tc.water_unit,tc.county,tm.line_num,tm.waters_type,tm.memo,tm.vender FROM t_water_meter tm " +
    ...
    public Page<ActualData> getActualDataPageByDisable(int page, int rows, String keyword, String orderbyStr) {
        ...
        StringBuffer sqlExceptSelect = new StringBuffer("from (SELECT tad.id,tc.inner_code,tc.real_code,tm.meter_address,tad.alarm,tad.net_water,tad.sum_water,tad.state," +
                "tad.write_time,tad.voltage,companyName,tc.water_unit,tc.county,tm.line_num,tm.waters_type,tm.memo,tm.vender FROM t_water_meter tm " +
                "left join (select name as companyName,water_unit,county,inner_code,real_code from t_company) tc on tm.inner_code=tc.inner_code " +
                "left join t_actual_data tad on tad.meter_address=tm.meter_address " +
                "where tm.meter_address not in (select meter_address from t_actual_data)) alld ");
        sqlExceptSelect.append(" where 1=1");
    ...
    public Page<ActualData> getActualDataPageByStatus(int page, int rows, String keyword, String orderbyStr, String status, int exceptionTime) {
        //select * from (select * from t_actual_data order by write_time desc) a group by a.meter_address order by write_time desc
        String select = "select * ";
        StringBuffer sqlExceptSelect = new StringBuffer("from (" +
                "select al.*," +
    ...
    public Page<ActualData> getReadnumStatis(int pageNo, int pageSize, String orderbyStr, Date startTime, Date endTime,
        ...
        if (StringUtils.isNotEmpty(meterAttr)) {
            ...
            if (StringUtils.isNotEmpty(meterAttr)) {
                sqlExceptSelect.append(" and tm.meter_attr='" + meterAttr + "'");
            }
        }
        ...
        if (StringUtils.isNotEmpty(companyType)) {
            companyType = StringUtils.trim(companyType);
            if (StringUtils.isNotEmpty(companyType)) {
                sqlExceptSelect.append(" and tc.company_type=" + companyType + " ");
            }
        }
    ...
    public List<Record> getCPAMonthActualData() {
        Map<String, String> map = ToolDateTime.getBefore12MonthDateTime();
        String start = map.get(MonthCode.warn_start_date);
        String end = map.get(MonthCode.warn_end_date);
        String sql = "select  sum(t.net_water) as sumWater,date_format(t.write_time, '%Y-%m') as month,t.* from t_actual_data t" +
