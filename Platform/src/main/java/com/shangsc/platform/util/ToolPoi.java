...
public abstract class ToolPoi {
	...
	private static final Log log = Log.getLog(ToolPoi.class);
	...
	public static String export(String templatePath) {
		...
		// 导出文件路径
		String path = filePath + File.separator + ToolDateTime.format(new Date(), "yyyyMMddHHmmss") + ".xlsx";
	...
	public static String writeExcel(SXSSFWorkbook wb, String name) {
		String filePath = PropKit.get("downloadPath");
		File f = new File(filePath);
		if (!f.exists()) {
			f.mkdir();
		}

		String path = filePath + File.separator + name + ToolDateTime.format(new Date(), "yyyyMMddHHmmss") + ".xlsx";
